...
function analyzeHTML(html, options = {}) {
  ...
  return {
    ...
    sections,
    productImages: extractProductCarouselImages($, pageUrl),
  };
}
...
function extractProductCarouselImages($, baseUrl) {
  const images = [];
  let origin = '';
  try { origin = new URL(baseUrl).origin; } catch (e) {}

  const firstEl = $('.carousel-item img.product_detail_img').first();
  if (firstEl.length) {
    const el = firstEl[0];
    const zoomImg = $(el).attr('data-zoom-image');
    const src = $(el).attr('src');
    const alt = $(el).attr('alt') || 'product_image';
    const srcset = $(el).attr('srcset') || $(el).parent().find('source').attr('srcset');
    ...
    if (srcset) {
      srcset.split(',').forEach(part => {
        ...
        if (urlMatch) {
          try {
            const absolute = new URL(urlMatch, baseUrl).href;
            srcsetUrls.push(absolute);
          } catch(e) {}
        }
      });
    }
    ...
    images.push({
      index: 1,
      alt,
      highRes: zoomImg ? new URL(zoomImg, baseUrl).href : null,
      standardRes: src ? new URL(src, baseUrl).href : null,
      allSources: Array.from(new Set(srcsetUrls)),
      originalZoom: zoomImg
    });
  }
  return images;
}

module.exports = { analyzeHTML, extractLinks, discoverStructure, extractProductCarouselImages };
